...
**规则**：
- HTML 里给人类用的「chrome 元素」（progress bar / replay button / counter / phase labels）和视频内容本体分开管理
- 不要把内容常用类名（`.title` / `.footer` / `.masthead` / `.kicker`）放进默认隐藏规则；需要隐藏的调试 UI 用 `.no-record`、`[data-role="chrome"]` 或 `[data-record="hidden"]` 显式标记。
