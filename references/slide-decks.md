...
- **无限画廊 gallery**：所有页**无缝无限平铺 + 缓慢漂移 + 轻微呼吸缩放**，一个 tile 含全部页（洗牌排布，看完所有页才重复）。瓦片多，**必须用 `<img>` 缩略图**扛性能（见下）；任一页缺 thumb 时自动回退到 grid，绝不在 gallery 批量创建 iframe。
...
3. **画廊必须完整缩略图**：`DECK_MANIFEST` 每一项都要有 `thumb` 才会启用 gallery；缺一张就回退 grid，避免数百个 iframe 把浏览器拖死。缩略图分辨率别太低：< 1000px hover 放大后会发虚，默认 1600px。
