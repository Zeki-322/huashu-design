...
- **无限画廊 gallery**：所有页**无缝无限平铺 + 缓慢漂移 + 轻微呼吸缩放**，一个 tile 含全部页（洗牌排布，看完所有页才重复）。瓦片多，**必须用 `<img>` 缩略图**扛性能（见下）；只要有任一页缺 thumb，`deck_index.html` 会自动回到 grid，避免 gallery 退化成海量 iframe。
