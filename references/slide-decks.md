...
- **无限画廊 gallery**：所有页**无缝无限平铺 + 缓慢漂移 + 轻微呼吸缩放**，一个 tile 含全部页（洗牌排布，看完所有页才重复）。瓦片多，**必须用 `<img>` 缩略图**扛性能（见下）；thumb 不完整时自动回退 grid，禁止在画廊里回退 iframe。
...
3. **gallery 必须有完整 thumb**：任何一页缺 thumb 都回退 grid，避免无限画廊创建几十/上百个 iframe 卡死浏览器。缩略图分辨率也别太低：< 1000px hover 放大后发虚，默认 1600px。
