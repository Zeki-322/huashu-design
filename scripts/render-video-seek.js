 ...
 * window.__seekRenderReady = true 与 window.__seek(t)。纯 CSS @keyframes / Lottie / 非 Stage 驱动的动画不吃 __seek，
...
async function renderFrames(context, url, frames) {
  ...
  // Stage / NarrationStage 在 __seekRender 模式下会暴露明确握手，证明自驱时钟已冻结。
...
(async () => {
  ...
  try {
    ...
    if (/__seek|__ready/.test(msg)) {
      ...
      console.error('  seek 渲染只支持走 Stage 时钟且能确认冻结自驱时钟的动画（assets/animations.jsx 的 <Stage>');
