...
// 只隐藏明确的控制条；不要使用 `.title`/`.footer` 这类内容常用类名。
...
async function renderFrames(context, url, frames) {
  ...
  // Stage / NarrationStage 在 __seekRender 模式下会暴露 window.__seek 并确认自驱时钟已冻结
...
(async () => {
  ...
  try {
    ...
    if (/__seek|__ready/.test(msg)) {
      console.error('');
      console.error('✗ 动画没有暴露安全的 window.__seekRenderReady/window.__seek（或未就绪）。');
  ...
  for (let f = 0; f < TOTAL_FRAMES; f++) {
    const framePath = path.join(TMP_DIR, 'frame-' + String(f).padStart(6, '0') + '.png');
    if (!fs.existsSync(framePath)) missingFrames.push(f);
  }
  if (pngCount !== TOTAL_FRAMES || missingFrames.length) {
    console.error(`✗ 截帧不完整：${pngCount}/${TOTAL_FRAMES}`);
    if (missingFrames.length) console.error('  缺失帧: ' + missingFrames.slice(0, 20).join(', ') + (missingFrames.length > 20 ? ' …' : ''));
    cleanupArtifacts();
    process.exit(1);
  }
