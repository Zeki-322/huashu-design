...
// 避免 broad content names like `.title` or `.footer`，它们常是画面主体。
...
// 一个 worker：开一个 page，goto，等 seek-render 冻结时钟握手就绪，渲染分配给它的帧
async function renderFrames(context, url, frames) {
  const page = await context.newPage();
  await page.goto(url, { waitUntil: 'load', timeout: 60000 });

  // Stage / NarrationStage 在 __seekRender 模式下必须暴露 window.__seekRenderReady，
  // 确认自驱 rAF 已冻结；仅有 __seek 的手写 demo 不能安全逐帧导出。
...
(async () => {
  ...
  try {
    ...
    if (/__seek|__seekRenderReady|__ready/.test(msg)) {
      console.error('');
      console.error('✗ 动画没有暴露冻结时钟 seek-render 握手（或未就绪）。');
