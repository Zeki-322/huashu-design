...
async function renderFrames(context, url, frames) {
  ...
  // Stage / NarrationStage 在 __seekRender 模式下会暴露 __seekRenderReady，
  // 证明它们已经冻结自驱时钟，后续每帧只由外部 __seek 推进。
...
(async () => {
  ...
  try {
    ...
    if (/Timeout|__seek|__seekRenderReady|__ready/.test(msg)) {
      console.error('');
      console.error('✗ 动画没有暴露冻结时钟握手（window.__seekRenderReady + window.__seek + window.__ready）。');
