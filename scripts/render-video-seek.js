 ...
 * 的 <NarrationStage>），它们会响应 window.__seekRender 冻结自驱时钟、暴露
 * window.__seek(t)，并设置 window.__seekRenderReady=true。纯 CSS @keyframes /
 * Lottie / 非 Stage 驱动的动画不吃 __seek，这类请继续用 render-video.js。
 *
...
async function renderFrames(context, url, frames) {
  ...
  // Stage / NarrationStage 在 __seekRender 模式下会暴露正向握手，证明自驱时钟已冻结。
...
(async () => {
  ...
  try {
    ...
    if (/__seek|__ready/.test(msg)) {
      console.error('');
      console.error('✗ 动画没有暴露冻结时钟握手 window.__seekRenderReady/window.__seek（或未就绪）。');
  ...
  if (pngCount !== TOTAL_FRAMES) {
    console.error(`✗ 帧数不完整：截到 ${pngCount}/${TOTAL_FRAMES} 帧`);
    fs.rmSync(TMP_DIR, { recursive: true, force: true });
    fs.rmSync(MP4_OUT, { force: true });
    process.exit(1);
  }
