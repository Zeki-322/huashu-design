...
// Keep this list explicit: generic content classes like .title/.footer are common slide content.
...
if (!Number.isFinite(DURATION) || DURATION <= 0 || !Number.isFinite(FPS) || FPS <= 0) {
  console.error('Duration and fps must be positive numbers.');
  process.exit(1);
}
...
async function renderFrames(context, url, frames) {
  ...
  // Stage / NarrationStage 在 __seekRender 模式下会暴露 window.__seek，
  // 并用 __seekRenderReady 确认自驱时钟已被冻结。
...
(async () => {
  ...
  try {
    ...
    if (/Timeout|__seek|__ready|__seekRenderReady/.test(msg)) {
      console.error('');
      console.error('✗ 动画没有暴露 window.__seekRenderReady / window.__seek（或未就绪）。');
  ...
  cleanupPartial();
  console.error(String(e && e.stack || e).slice(0, 2000));
  process.exit(1);
});
