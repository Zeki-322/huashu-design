...
async function renderFrames(context, url, frames) {
  ...
  // 并用 __seekRenderReady 明确确认自驱时钟已冻结。
...
(async () => {
  ...
  try {
    ...
    if (/__seek|__ready|Timeout/.test(msg)) {
      ...
      console.error('  并设置 window.__seekRenderReady = true 的 Stage / NarrationStage。');
      console.error('  纯 CSS @keyframes / Lottie / 手写非冻结动画请改用 render-video.js。');
      console.error('');
    }
    await browser.close();
  ...
  if (pngCount !== TOTAL_FRAMES) {
    failAndClean(`✗ 帧数不完整：截到 ${pngCount}/${TOTAL_FRAMES} 帧，停止编码以避免静默短片。`);
  }
