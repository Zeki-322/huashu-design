...
test('voiceover ducking keeps voice usable for both sidechain and final mix', () => {
  const src = read('scripts/mix-voiceover.sh');
  assert.match(src, /asplit=2\[voice_sc\]\[voice_mix\]/);
  assert.match(src, /\[bgm_lo\]\[voice_sc\]sidechaincompress/);
  assert.match(src, /\[voice_mix\]\[bgm_ducked\]amix/);
