...
async function main() {
  ...
  if (errors.length) {
    ...
    if (errors.length === files.length || !allowPartial) {
      const scope = errors.length === files.length ? '全部失败' : '部分失败';
      console.error(`✗ ${scope}，不生成 PPTX。若确实需要缺页文件，请显式传 --allow-partial。`);
      await fs.rm(outFile, { force: true });
      process.exit(1);
    }
    console.error(`⚠️ 已显式允许 --allow-partial，将生成缺页 PPTX。`);
  }
