...
function parseArgs() {
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    } else if (a[i + 1] && !a[i + 1].startsWith('--')) {
      args[k] = a[++i];
    } else {
      args[k] = true;
    }
...
async function removeOutput(outFile) {
  try {
    await fs.rm(outFile, { force: true });
  } catch (e) {
    console.error(`✗ 清理不完整 PPTX 失败：${outFile}`);
  ...
  if (errors.length) {
    ...
    if (!allowPartial || errors.length === files.length) {
      console.error(errors.length === files.length
        ? `✗ 全部失败，不生成 PPTX。`
        : `✗ 默认不生成缺页 PPTX；如确需部分输出，请显式传 --allow-partial。`);
      await removeOutput(outFile);
      process.exit(1);
    }
    console.error(`⚠️ 已显式允许部分输出（--allow-partial），将生成缺少 ${errors.length} 张 slide 的 PPTX。`);
  }
