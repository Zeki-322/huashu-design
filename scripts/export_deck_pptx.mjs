...
async function main() {
  ...
  if (errors.length) {
    ...
    if (!allowPartial || errors.length === files.length) {
      ...
      await fs.rm(tmpOutFile, { force: true }).catch(() => {});
      const reason = errors.length === files.length ? '全部失败' : '默认禁止缺页 PPTX';
      console.error(`✗ ${reason}，不生成 PPTX。若确认要交付缺页文件，请显式传 --allow-partial。`);
      process.exit(1);
    }
    console.error(`⚠️ 已显式传 --allow-partial，将生成缺页 PPTX。`);
  }
