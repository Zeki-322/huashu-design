...
if [ -z "$BGM" ]; then
  ...
  FADE_START=$(ffprobe -v error -show_entries format=duration -of default=nw=1:nk=1 "$INPUT" | awk '{ v=$1-0.5; if (v < 0) v=0; printf "%.3f", v }')
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      [1:a]volume=${VOICE_VOLUME},asplit=2[voice_sc][voice_mix];
