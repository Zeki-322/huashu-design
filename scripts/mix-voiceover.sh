...
if [ -z "$BGM" ]; then
  ...
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      [1:a]volume=${VOICE_VOLUME},apad,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB,asplit=2[voice_mix][voice_sc];
      ...
      [voice_mix][bgm_ducked]amix=inputs=2:duration=first:dropout_transition=0,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[a]
    " \
    -map 0:v -map "[a]" \
    -c:v copy -c:a aac -b:a 192k \
    "$OUTPUT"
  ...
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      ...
      [voice][bgm]amix=inputs=2:duration=first:dropout_transition=0,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[a]
    " \
    -map 0:v -map "[a]" \
    -c:v copy -c:a aac -b:a 192k \
    "$OUTPUT"
