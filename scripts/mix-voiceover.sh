...
FADE_OUT_DUR="0.5"
FADE_OUT_START="$(awk -v d="$VIDEO_DURATION" -v f="$FADE_OUT_DUR" 'BEGIN { s=d-f; if (s<0) s=0; printf "%.3f", s }')"
...
if [ -z "$BGM" ]; then
  ...
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      ...
      [voice_mix][bgm_ducked]amix=inputs=2:duration=longest:dropout_transition=0,atrim=0:${VIDEO_DURATION},afade=t=out:st=${FADE_OUT_START}:d=${FADE_OUT_DUR}:curve=tri[a]
    " \
    -map 0:v -map "[a]" \
    -c:v copy -c:a aac -b:a 192k \
    "$OUTPUT"
