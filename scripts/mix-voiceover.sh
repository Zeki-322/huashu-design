...
if ! node -e "const d=Number(process.argv[1]); process.exit(Number.isFinite(d) && d > 0 ? 0 : 1)" "$VIDEO_DURATION"; then
  echo "✗ 无法读取视频时长: $INPUT" >&2
  exit 1
fi
FADE_DURATION="0.5"
FADE_OUT_START="$(node -e "const d=Number(process.argv[1]); const f=Number(process.argv[2]); console.log(Math.max(0, d - f).toFixed(3))" "$VIDEO_DURATION" "$FADE_DURATION")"
...
if [ -z "$BGM" ]; then
  ...
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      [1:a]volume=${VOICE_VOLUME},apad,atrim=0:${VIDEO_DURATION},asplit=2[voice_sc][voice_mix];
      ...
      [voice_mix][bgm_ducked]amix=inputs=2:duration=longest:dropout_transition=0,atrim=0:${VIDEO_DURATION},afade=t=out:st=${FADE_OUT_START}:d=${FADE_DURATION}:curve=tri[a]
    " \
    -map 0:v -map "[a]" \
    -c:v copy -c:a aac -b:a 192k -t "$VIDEO_DURATION" \
    "$OUTPUT"
