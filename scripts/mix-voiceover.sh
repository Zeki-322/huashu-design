...
VIDEO_DURATION="$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$INPUT" 2>/dev/null || true)"
...
if [ -z "$BGM" ]; then
  ...
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" \
    -filter_complex "[1:a]volume=${VOICE_VOLUME},apad,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[a]" \
    -map 0:v -map "[a]" \
    -c:v copy -c:a aac -b:a 192k -t "$VIDEO_DURATION" \
    "$OUTPUT"
elif [ "$DUCKING" = "1" ]; then
  # 人声 + BGM + sidechain ducking
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      [1:a]volume=${VOICE_VOLUME},apad,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[voice_full];
      [voice_full]asplit=2[voice_mix][voice_side];
      [2:a]volume=${BGM_VOLUME},aloop=loop=-1:size=2e9,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[bgm_lo];
      [bgm_lo][voice_side]sidechaincompress=threshold=0.04:ratio=8:attack=5:release=300:makeup=1[bgm_ducked];
  ...
  ffmpeg -y -i "$INPUT" -i "$VOICEOVER" -i "$BGM" \
    -filter_complex "
      [1:a]volume=${VOICE_VOLUME},apad,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[voice];
      [2:a]volume=${BGM_VOLUME},aloop=loop=-1:size=2e9,atrim=0:${VIDEO_DURATION},asetpts=N/SR/TB[bgm];
