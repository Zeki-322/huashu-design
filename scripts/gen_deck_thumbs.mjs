 ...
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。缺少 thumb 时画廊会回退 grid。
 *
...
for (const f of files) {
  ...
  try {
    ...
    try { if (fs.existsSync(out)) fs.rmSync(out, { force: true }); } catch (_) {}
    failures.push(f);
...
if (failures.length) {
  console.error('缩略图生成失败，已移除失败页的旧输出: ' + failures.join(', '));
  process.exit(1);
}
