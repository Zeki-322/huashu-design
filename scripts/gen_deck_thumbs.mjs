 ...
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。缩略图不完整时不要启用画廊。
 *
...
for (const f of files) {
  ...
  const tmpOut = out + '.tmp-' + process.pid + '.jpg';
  try {
    ...
    await sharp(buf).resize(width).jpeg({ quality }).toFile(tmpOut);
...
if (failures.length) {
  console.error('✗ 缩略图生成不完整，已删除失败页的旧输出。请修复后重跑。');
  process.exit(1);
}
