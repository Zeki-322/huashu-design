 ...
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。没有完整 thumb 时会自动降级网格。
 *
...
for (const f of files) {
  ...
  const tmp = path.join(outDir, `.${base}.${process.pid}.tmp.jpg`);
  try {
    ...
    await sharp(buf).resize(width).jpeg({ quality }).toFile(tmp);
...
if (failures.length) {
  console.error('✗ 缩略图生成不完整，未覆盖失败页的旧文件：');
  for (const { file, error } of failures) console.error(`  - ${file}: ${error}`);
  process.exit(1);
}
