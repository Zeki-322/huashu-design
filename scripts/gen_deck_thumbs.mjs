 ...
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。thumb 不完整时画廊会回退网格。
 *
...
if (!Number.isFinite(quality) || quality <= 0 || quality > 100) { console.error('无效 --quality: ' + arg('quality', '86')); process.exit(1); }
if (!Number.isFinite(W) || W <= 0 || !Number.isFinite(H) || H <= 0) { console.error('无效 canvas 尺寸'); process.exit(1); }
