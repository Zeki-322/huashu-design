 ...
 *   node gen_deck_thumbs.mjs --slides slides --out thumbs [--index index.html] [--width 1600] [--quality 86]
 *
 * 然后在 index.html 的 MANIFEST 给每项加 thumb（与 file 同名 .jpg）：
 *   { file: "slides/01-cover.html", thumb: "thumbs/01-cover.jpg", label: "封面" }
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。没有完整 thumb 时概览会回退 grid。
 *
...
const entries = await resolveDeckEntries({ slidesDir, indexPath });
...
for (const entry of entries) {
  ...
  try {
    await page.goto(pathToFileURL(entry.absPath).href, { waitUntil: 'load' });
    ...
    console.error('[FAIL] ' + entry.displayName + ': ' + e.message);
  }
}
await browser.close();
console.log(`\n=== ${ok}/${entries.length} 张缩略图 → ${outDir}/ ===`);
console.log('在 index.html 的 MANIFEST 每项加 thumb: "' + outDir + '/<同名>.jpg"（仅画廊模式用到）');
if (errors.length) {
  console.error(`✗ ${errors.length} 张缩略图生成失败；已删除对应旧缩略图，避免 gallery 使用过期/缺失图片。`);
  process.exit(1);
}
