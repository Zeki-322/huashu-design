 ...
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。缩略图不完整时画廊会降级 grid。
 *
...
for (const slide of files) {
  const base = path.basename(slide.file).replace(/\.html$/, '');
  const out = path.join(outDir, base + '.jpg');
  try {
    await page.goto('file://' + slide.path, { waitUntil: 'load' });
