 ...
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。没有完整 thumb 时画廊回退网格。
 *
...
if (!Number.isFinite(width) || width <= 0) { console.error('width 必须是正数'); process.exit(1); }
if (!Number.isFinite(quality) || quality < 1 || quality > 100) { console.error('quality 必须在 1..100'); process.exit(1); }
if (!Number.isFinite(W) || W <= 0 || !Number.isFinite(H) || H <= 0) { console.error('canvas-w/canvas-h 必须是正数'); process.exit(1); }
...
if (failures.length) {
  console.error('✗ 缩略图生成不完整，已删除失败页的旧缩略图：' + failures.join(', '));
  process.exit(1);
}
