...
if (!isPositiveInt(width) || quality < 1 || quality > 100 || !isPositiveInt(W) || !isPositiveInt(H)) {
  console.error('参数无效：--width/--canvas-w/--canvas-h 必须为正整数，--quality 必须在 1-100。');
  process.exit(1);
}
...
if (failed) {
  console.error(`✗ ${failed} 张缩略图生成失败；已删除失败页的旧缩略图。`);
  process.exit(1);
}
