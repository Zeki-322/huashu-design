...
if (!Number.isFinite(quality) || quality < 1 || quality > 100) { console.error('quality 必须是 1-100 的整数'); process.exit(1); }
...
if (failures.length) {
  console.error(`✗ ${failures.length} 张缩略图生成失败；请修复后重跑，避免 gallery 缺图回退。`);
  process.exit(1);
}
