 ...
 *   node gen_deck_thumbs.mjs --slides slides --out thumbs [--width 1600] [--quality 86] [--timeout 30000]
 *
 * 然后在 index.html 的 MANIFEST 给每项加 thumb（与 file 同名 .jpg）：
 *   { file: "slides/01-cover.html", thumb: "thumbs/01-cover.jpg", label: "封面" }
 * deck_index.html 仅在画廊模式用 thumb；网格模式始终用 file(iframe)。没有完整 thumb 时画廊会降级为 grid。
 *
...
for (const f of files) {
  ...
  try {
    await page.goto('file://' + path.resolve(slidesDir, f), { waitUntil: 'load', timeout });
...
if (failures.length) {
  console.error('\n✗ 缩略图生成不完整，未生成完整 thumb 时不能开启 gallery：');
