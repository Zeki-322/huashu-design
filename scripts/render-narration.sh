...
if [ -n "$USE_SEEK" ]; then
  ...
  NODE_PATH="$(cd "$SKILL_ROOT" && npm root)${NODE_PATH:+:$NODE_PATH}" node "$SCRIPT_DIR/render-video-seek.js" "$HTML_ABS" \
  ...
  NODE_PATH="$(cd "$SKILL_ROOT" && npm root)${NODE_PATH:+:$NODE_PATH}" node "$SCRIPT_DIR/render-video.js" "$HTML_ABS" \
    --duration="$RECORD_DURATION" \
    --width="$WIDTH" \
    --height="$HEIGHT"
fi
